...
def runKBudget(timestamps, k, maxiters = 10):
    """
...
if __name__ == "__main__":
 ...
 #   k = 10
    ...
    #nodeEdgeIndex = utils.getIndex(timestamps, 'budget')
    #m = getTrueGapPoints(nodeEdgeIndex, timestamps[0][0], timestamps[-1][0], active_truth)
    #print m
    #m = getGapPoints(nodeEdgeIndex, timestamps[0][0], timestamps[-1][0], k)
    ##print m
    #exit()
    #Xstart, Xend = runKBudget(timestamps, nodeEdgeIndex, b = {node: [event_length-1]*k for node in active_truth}, m = m, k = k)
    #Xstart, Xend = runKBudget(timestamps, nodeEdgeIndex, b = {node: [1]*k for node in active_truth}, m = {}, k = k)
    #Xstart, Xend = runKBudget_BS(timestamps,nodeEdgeIndex, k, maxiters = 100, m = m)
    Xstart, Xend = runKBudget(timestamps, k, maxiters = 10)
    ...
    #print 'relative total length of solution =', utils.getCost(Xstart, Xend), (1.0*k*(event_length-1)*num_nodes)
    #print 'relative maximum length of solution =', utils.getMax(Xstart, Xend), (1.0*event_length-1)
